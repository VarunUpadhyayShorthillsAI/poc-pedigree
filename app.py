    ...
    try:
        ...
        if lines is not None and len(lines) > 0:
            ...
            if angles:
                ...
                app_logger.info(f"Detected document rotation angle: {detected_angle:.2f} degrees")
                ...
                app_logger.info(f"Corrected angle after normalization: {corrected_angle:.2f} degrees")
        ...
        try:
            ...
            if ocr_angle not in [0, None]:
                app_logger.info(f"OCR detected orientation adjustment: {ocr_angle} degrees")
                if abs(ocr_angle) >= 5:
                    (h, w) = img.shape[:2]
                    center = (w // 2, h // 2)
            ...
            app_logger.warning(f"OCR orientation detection failed (continuing without it): {e}")
